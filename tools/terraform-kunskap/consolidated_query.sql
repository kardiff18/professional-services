...
(
  ...
  combined_eu_us_projects AS (
    SELECT *
    FROM
      `{billing_project_id}.CorrectedBillingData.{project_breakdown_table}_us`
    UNION ALL
    SELECT *
    FROM
      `{billing_project_id}.CorrectedBillingData.{project_breakdown_table}_eu`
  ),
  projects_with_names AS (
    SELECT start_day, name_table.project_id, slot_days, usage_percent, project_name
    FROM combined_eu_us_projects AS data_table
